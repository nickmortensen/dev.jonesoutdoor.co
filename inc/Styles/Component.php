...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'wp-rig-fonts', $google_fonts_url, [], $version, 'all' ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
	...
	protected function get_css_files() : array {
		...
		$css_files = [
			...
			// ],
			// 'google-map'           => [
			// 	'file'             => 'google-map.min.css',
			// 	'preload_callback' => function() {
			// 		global $template;
			// 		return 'frontpage-map.php' === basename( $template );
			// 	},
			// ],
			// phpcs:enable
			'wp-rig-global'     => [
